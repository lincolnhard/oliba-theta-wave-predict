...
?>
<!-- <audio id="music-obj" src="/_asset/music/music.mp3" loop></audio> -->
<h3>&nbsp;</h3>
<div id="test-section" style="display: none;">
    <div style="clear: both; margin-top: 100px;">
        <h3 class="rule-1-text" style="display: none; text-align: center;"><?php echo $rule_1; ?></h3 >
        <h3 class="rule-2-text" style="display: none; text-align: center;"><?php echo $rule_2; ?></h3 >
...
<form id="test-form" action="/pretest-result.php" method="post">
...
$(document).ready(function() {
    ...
    function render_pictures() {
        ...
        $('#left-pointer').hide();
        $('#right-pointer').hide();
        if (set > <?php echo 4 - 2; ?> && previous_wrong) {
...
case 'sharp-and-round':
    $prefix_1       = $_SERVER['DOCUMENT_ROOT'].'/_data/sharp/';
    $prefix_2       = $_SERVER['DOCUMENT_ROOT'].'/_data/round/';
    $rule_1         = 'Which picture is "Sharper"?';
    $rule_2         = 'Which picture is "Rounder"?';
    $rest_content_1 = '"sharp and round" rest content 1';
    $rest_content_2 = '"sharp and round" rest content 2';
    $rest_content_3 = '"sharp and round" rest content 3';
    $rest_content_4 = '"sharp and round" rest content 4';
    break;
case 'mood-distingulish':
    $prefix_1       = $_SERVER['DOCUMENT_ROOT'].'/_data/positive/';
    $prefix_2       = $_SERVER['DOCUMENT_ROOT'].'/_data/negative/';
    $rule_1         = 'Which picture is "Positive"?';
    $rule_2         = 'Which picture is "Negative"?';
    $rest_content_1 = '"mood distingulish" rest content 1';
    $rest_content_2 = '"mood distingulish" rest content 2';
    $rest_content_3 = '"mood distingulish" rest content 3';
    $rest_content_4 = '"mood distingulish" rest content 4';
    break;
}
...
<div id="rest-section-3" style="display: none;">
    <p><?php echo $rest_content_3; ?></p>
    <button type="button" class="btn btn-primary btn-lg pull-right rest-btn">Continue</button>
    <div style="clear: both;"></div>
</div>
<div id="rest-section-4" style="display: none;">
    <p><?php echo $rest_content_4; ?></p>
    <button type="button" class="btn btn-primary btn-lg pull-right rest-btn">Continue</button>
    <div style="clear: both;"></div>
</div>
<form id="pretest-form" action="/pretest-result.php" method="post">
    <input type="hidden" name="tester" value="<?php echo $_POST['tester']; ?>" />
    <input type="hidden" name="mode" value="<?php echo $_POST['mode']; ?>" />
    <input type="hidden" name="type" value="<?php echo $_POST['type']; ?>" />
    <input type="hidden" name="total_set" value="<?php echo $_POST['total_set']; ?>" />
...
$(document).ready(function() {
    ...
    var rule_1 = '<?php echo $rule_1; ?>';
    var rule_2 = '<?php echo $rule_2; ?>';
    ...
    function render_pictures() {
        ...
        if (current_rule == 1) {
            tag_1 = 'c';
            tag_2 = 'w';
            $('#rule-block').html(rule_1);
        } else {
            tag_1 = 'w';
            tag_2 = 'c';
            $('#rule-block').html(rule_2);
        }
        if (Math.floor((Math.random() * 2) + 1) == 1) {
            $('#left-link').attr('data-tag', tag_1);
            $('#left-link > img').attr('src', prefix_1 + pool_1[random_1]);
    ...
    $(document.body).on('click', '.answer-link', function() {
        if ($('#test-section').css('display') != 'none') {
            ...
            response.push([$(this).attr('data-tag')]);
            if (counter >= 5) {
                counter = 0;
                if (set == 4) {
                    $('input[name="response"]').val(JSON.stringify(response).replace(/\"/g, "'"));
